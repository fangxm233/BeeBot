...
export class Visualizer {
	...
	public static circle(pos: RoomPosition, color = 'red', opts = {}): RoomVisual {
		_.defaults(opts, {
			fill   : color,
			radius : 0.35,
			opacity: 0.5,
	...
	public static marker(pos: RoomPosition, opts = {}): RoomVisual {
	...
	public static drawRoads(positoins: RoomPosition[]): void {
	...
	public static drawPath(path: RoomPosition[], style?: PolyStyle): void {
	...
	public static displayCostMatrix(costMatrix: CostMatrix, roomName?: string, dots = true, color = '#ff0000'): void {
	...
	public static showInfo(info: string[], calledFrom: { room: Room | undefined, pos: RoomPosition }, opts = {}): RoomVisual {
		if (calledFrom.room) {
	...
	public static section(title: string, pos: { x: number, y: number, roomName?: string }, width: number,
	...
	public static infoBox(header: string, content: string[] | string[][], pos: { x: number, y: number, roomName?: string },
				   width: number): number {
		// const vis = new RoomVisual(pos.roomName);
	...
	public static text(text: string, pos: { x: number, y: number, roomName?: string }, size = 1, style: TextStyle = {}): void {
		new RoomVisual(pos.roomName).text(text, pos.x, pos.y, this.textStyle(size, style));
	}

	public static barGraph(progress: number | [number, number], pos: { x: number, y: number, roomName?: string },
	...
	public static table(data: string[][], pos: { x: number, y: number, roomName?: string }): void {
		if (data.length == 0) {
			return;
		}
		const colPadding = 4;
	...
	public static multitext(lines: string[], pos: { x: number, y: number, roomName?: string }): void {
		if (lines.length == 0) {
			return;
		}
		const vis = new RoomVisual(pos.roomName);
	...
	public static drawNotifications(notificationMessages: string[]): void {
